...
from app.agents.legal.legal_manager import legal_manager_agent
...
def run_contract_review(task_name: str, payload: dict):
    t = Task(
        ...
        agent=legal_manager_agent
    )
    crew = Crew(
        agents=[legal_manager_agent, contract_review_agent], 
        tasks=[t],
        verbose=True,
        planning=True
    )
...
def run_compliance_monitoring(task_name: str, payload: dict):
    ...
    crew = Crew(
        agents=[legal_manager_agent, regulatory_compliance_agent], 
        tasks=[t],
        verbose=True,
        planning=True
    )
...
def run_litigation_support(task_name: str, payload: dict):
    ...
    crew = Crew(
        agents=[legal_manager_agent, litigation_support_agent], 
        tasks=[t],
        verbose=True,
        planning=True
    )
