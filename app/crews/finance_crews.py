...
from app.agents.finance.finance_manager import finance_manager_agent
...
def run_accounts_payable(task_name: str, payload: dict):
    t = Task(
        ...
        agent=finance_manager_agent
    )
    crew = Crew(
        agents=[finance_manager_agent, accounts_payable_agent], 
        tasks=[t],
        verbose=True,
        planning=True
    )
...
def run_accounts_receivable(task_name: str, payload: dict):
    ...
    crew = Crew(
        agents=[finance_manager_agent, accounts_receivable_agent], 
        tasks=[t],
        verbose=True,
        planning=True
    )
...
def run_finance_risk(task_name: str, payload: dict):
    ...
    crew = Crew(
        agents=[finance_manager_agent, finance_risk_agent], 
        tasks=[t],
        verbose=True,
        planning=True
    )
